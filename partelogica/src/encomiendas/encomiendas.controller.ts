...
export class EncomiendasController {
  ...
  @Post()
  @ApiOperation({ summary: 'Crear una nueva encomienda con sus detalles y seguro de forma transaccional y atómica' })
  @ApiResponse({ status: 201, description: 'Encomienda, detalles y seguro creados exitosamente en una única transacción.' })
  @ApiResponse({ status: 400, description: 'Código duplicado, falta de detalles u otros errores de validación.' })
  @ApiResponse({ status: 404, description: 'Clientes o tipo de paquete no encontrados.' })
  ...
  @ApiResponse({ status: 200, description: 'Lista de encomiendas devuelta exitosamente.' })
  findAll() {
  ...
  @ApiResponse({ status: 200, description: 'Encomienda devuelta exitosamente.' })
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Actualizar los datos básicos o seguro de una encomienda' })
  @ApiResponse({ status: 200, description: 'Encomienda actualizada exitosamente.' })
  @ApiResponse({ status: 404, description: 'Encomienda no encontrada.' })
  update(@Param('id', ParseIntPipe) id: number, @Body() updateEncomiendaDto: UpdateEncomiendaDto) {
