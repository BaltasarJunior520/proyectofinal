...
export class FinanzasController {
  ...
  @Post('pagos')
  @ApiOperation({ summary: 'Registrar un pago y opcionalmente generar una factura comercial de forma uno a uno' })
  @ApiResponse({ status: 201, description: 'Pago registrado y factura creada exitosamente.' })
  ...
  @ApiResponse({ status: 200, description: 'Lista de pagos devuelta exitosamente.' })
  findAllPagos() {
  ...
  @Get('facturas')
  @ApiOperation({ summary: 'Obtener la lista de todas las facturas comerciales emitidas' })
  @ApiResponse({ status: 200, description: 'Lista de facturas devuelta exitosamente.' })
  findAllFacturas() {
    return this.finanzasService.findAllFacturas();
  }

  @Get('facturas/:id')
  @ApiOperation({ summary: 'Obtener una factura comercial específica por su ID' })
