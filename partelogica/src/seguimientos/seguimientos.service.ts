...
export class SeguimientosService {
  ...
  async create(
    ...
    const envio = await this.enviosRepository.findOne({
      where: { id: createSeguimientoDto.envioId },
    });
    if (!envio) {
      throw new NotFoundException(
        `El envío con ID ${createSeguimientoDto.envioId} no existe.`,
      );
    }

    const estado = await this.estadosRepository.findOne({
      where: { id: createSeguimientoDto.estadoId },
    });
    if (!estado) {
      throw new NotFoundException(
        `El estado de envío con ID ${createSeguimientoDto.estadoId} no existe.`,
      );
    }

    const seguimiento =
      this.seguimientosRepository.create(createSeguimientoDto);
    const savedSeguimiento =
  ...
  async findOne(id: number): Promise<Seguimiento> {
    const seguimiento = await this.seguimientosRepository.findOne({
      where: { id },
      relations: { envio: true, estado: true },
    });
    if (!seguimiento) {
  ...
  async findByEnvioId(envioId: number): Promise<Seguimiento[]> {
    const exists = await this.enviosRepository.findOne({
      where: { id: envioId },
    });
    if (!exists) {
